...
set :stages, %w(prototype production)
...
namespace :deploy do
  ...
  task :stop, :roles => :app, :except => { :no_release => true } do 
    run "cd #{current_path} && #{try_sudo} kill `cat #{unicorn_pid}`"
  end
  task :graceful_stop, :roles => :app, :except => { :no_release => true } do
    run "cd #{current_path} && #{try_sudo} kill -s QUIT `cat #{unicorn_pid}`"
  end
  task :reload, :roles => :app, :except => { :no_release => true } do
    run "cd #{current_path} && #{try_sudo} kill -s USR2 `cat #{unicorn_pid}`"
  end
